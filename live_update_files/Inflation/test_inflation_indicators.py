#!/usr/bin/env python3
"""
验证Inflation指标的FRED数据可用性
基于实际使用的test_banking_sector_indicators.py架构
严格遵循实时指标实施模板

Inflation指标列表(确保8个指标):
1. Consumer Price Index (CPI) - CPIAUCSL
2. Core PCE Price Index - PCEPILFE
3. Federal Funds Rate - FEDFUNDS
4. Unemployment Rate - UNRATE
5. Retail Sales - RSAFS
6. Bloomberg Commodity Index (BCOM) - 需要找替代指标 DJP 或 DJPNTR
7. 10-Year Breakeven Inflation Rate - T10YIEM
8. Global Supply Chain Pressure Index (GSCPI) - GSCPI
"""
...
def validate_business_rules(series_id, values):
    ...
    elif series_id in ['CPIAUCSL', 'PCEPILFE']:  # 价格指数
        if min_val < 0:
    ...
    elif series_id == 'T10YIEM':  # 通胀预期
        if min_val < -5 or max_val > 15:
            print(f"   ⚠️  通胀预期范围异常: {min_val}% - {max_val}%")
        else:
            print(f"   ✅ 通胀预期范围合理")
            
    elif series_id == 'RSAFS':  # 零售销售
        if min_val < 0:
            print(f"   ❌ 检测到负零售销售: {min_val}")
        else:
            print(f"   ✅ 零售销售数值范围合理")
            
    elif series_id == 'PPIACO':  # 生产者价格指数
        if min_val < 0:
            print(f"   ❌ 检测到负价格指数: {min_val}")
        else:
            print(f"   ✅ 生产者价格指数范围合理")
            
    elif series_id == 'DCOILWTICO':  # 原油价格
        if min_val < 0:
            print(f"   ❌ 检测到负油价: {min_val}")
        elif max_val > 200:
            print(f"   ⚠️  检测到异常高油价: {max_val}")
        else:
            print(f"   ✅ 原油价格范围合理")
...
def main():
    """主验证流程"""
    print("=== Inflation 指标验证 ===")
    print("基于FRED API的数据可用性和质量验证")
    
    # 定义要验证的指标列表 - 确保8个指标符合要求
    indicators = [
        ('CPIAUCSL', 'Consumer Price Index (CPI) - 消费者价格指数'),
        ('PCEPILFE', 'Core PCE Price Index - 核心PCE价格指数'),
        ('FEDFUNDS', 'Federal Funds Rate - 联邦基金利率'),
        ('UNRATE', 'Unemployment Rate - 失业率'),
        ('RSAFS', 'Retail Sales - 零售销售'),
        ('PPIACO', 'Producer Price Index: All Commodities - 生产者价格指数(商品)'),
        ('T10YIEM', '10-Year Breakeven Inflation Rate - 10年盈亏平衡通胀率'),
        ('DCOILWTICO', 'Crude Oil Prices (WTI) - 原油价格(替代供应链指数)'),
    ]
