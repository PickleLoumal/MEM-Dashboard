...
def get_minute_data(symbol: str, interval: str = '1m', adjust: str = '') -> pd.DataFrame:
    ...
    try:
        ...
        logger.error("AkShare minute data request failed for %s: %s", symbol, exc, exc_info=True)
        return pd.DataFrame()
...
def get_daily_data(symbol: str, start: datetime, end: datetime, adjust: str = '') -> pd.DataFrame:
    ...
    if df_raw is None or df_raw.empty:
        try:
            ...
            logger.error("AkShare daily data request failed for %s: %s", symbol, exc, exc_info=True)
            return pd.DataFrame()
