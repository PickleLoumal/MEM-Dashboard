...
class StockScore(models.Model):
    ...
    buy_reasons = models.JSONField(
        default=list, blank=True, help_text="Triggered buy signal reasons"
    )
    sell_score = models.PositiveSmallIntegerField(default=0, help_text="Sell signal score (0-100)")
    sell_reasons = models.JSONField(
        default=list, blank=True, help_text="Triggered sell signal reasons"
    )
    total_score = models.DecimalField(
        max_digits=6, decimal_places=2, default=0, help_text="Unified score (-100 to +100)"
    )
    score_components = models.JSONField(
        default=dict, blank=True, help_text="Component breakdown with raw/weighted contributions"
    )
    last_close = models.DecimalField(
        max_digits=20, decimal_places=6, blank=True, null=True, help_text="Previous close price"
    )
    last_trading_date = models.DateField(
        blank=True, null=True, help_text="Last trading date in the dataset"
    )
    cmf_value = models.DecimalField(
        max_digits=12, decimal_places=6, blank=True, null=True, help_text="Latest CMF value"
    )
    obv_value = models.BigIntegerField(blank=True, null=True, help_text="Latest OBV value")
    ma5 = models.DecimalField(
        max_digits=20, decimal_places=6, blank=True, null=True, help_text="MA5 value"
    )
    ma10 = models.DecimalField(
        max_digits=20, decimal_places=6, blank=True, null=True, help_text="MA10 value"
    )
    recommended_action = models.CharField(
        max_length=50, blank=True, help_text="Resulting recommended action"
    )
    recommended_action_detail = models.TextField(
        blank=True, help_text="Detailed recommended action explanation"
    )
    signal_date = models.DateField(blank=True, null=True, help_text="Signal generation date")
    execution_date = models.DateField(
        blank=True, null=True, help_text="Suggested execution date (typically T+1)"
    )
    suggested_position_pct = models.DecimalField(
        max_digits=6,
        decimal_places=4,
        blank=True,
        null=True,
        help_text="Suggested position percentage (0-1)",
    )
    stop_loss_price = models.DecimalField(
        max_digits=20, decimal_places=6, blank=True, null=True, help_text="Stop-loss trigger price"
    )
    take_profit_price = models.DecimalField(
        ...
        blank=True,
        null=True,
        help_text="Take-profit trigger price",
    )
...
class ScoreCalculationLog(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="pending", help_text="Status of the batch"
    )
    total_stocks = models.PositiveIntegerField(default=0, help_text="Number of stocks processed")
    successful_stocks = models.PositiveIntegerField(
        default=0, help_text="Number of successful stock updates"
    )
    failed_stocks = models.PositiveIntegerField(
        default=0, help_text="Number of stocks that failed to process"
    )
    processed_stocks = models.PositiveIntegerField(
        default=0, help_text="Number of stocks processed so far"
    )
    current_stock = models.CharField(
        max_length=50, blank=True, help_text="Currently processing stock symbol"
    )
