...
import time
import sys
from stocks.models import StockSymbol, StockUpdateLog
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['initialize']:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✅ Initialized {initialized} stock symbols')
            )
        ...
        if options['once']:
            ...
            if result['errors']:
                self.stdout.write(
                    self.style.WARNING(f'❌ Errors encountered:')
                )
        ...
        self.stdout.write(
            f'🚀 Starting continuous updates for {duration} seconds '
        ...
        self.stdout.write(
            self.style.SUCCESS('✅ Stock data updates completed')
        )
