...
class BaseDynamicConfigManager(ABC):
    ...
    def get_all_indicators(cls):
        ...
        try:
            ...
            logger.error(f"Error loading {cls.service_name} configs from database: {e}")
    ...
    def get_auto_fetch_indicators(cls):
        ...
        try:
            ...
            logger.error(f"Error loading {cls.service_name} auto-fetch configs: {e}")
            return {}
    ...
    def get_indicator_config(cls, series_id: str):
        ...
        try:
            config = cls.config_model.objects.filter(
                series_id=series_id, 
                is_active=True
            ).first()
            ...
            logger.error(f"Error loading {cls.service_name} config for {series_id}: {e}")
            return None
    ...
    def get_configs_by_category(cls, category: str):
        ...
        try:
            ...
            logger.error(f"Error loading {cls.service_name} configs for category {category}: {e}")
            return {}
    ...
    def create_indicator(cls, config_data: dict):
        ...
        try:
            ...
            logger.error(f"Error creating {cls.service_name} indicator config: {e}")
            raise
    ...
    def update_indicator(cls, series_id: str, config_data: dict):
        ...
        try:
            ...
            logger.error(f"{cls.service_name} indicator config not found: {series_id}")
            return None
        except Exception as e:
            logger.error(f"Error updating {cls.service_name} indicator config {series_id}: {e}")
            raise
    ...
    def get_statistics(cls) -> dict:
        ...
        try:
            ...
            auto_fetch = cls.config_model.objects.filter(
                is_active=True, 
                auto_fetch=True
            ).count()
            
            categories = cls.config_model.objects.filter(
                is_active=True
            ...
            logger.error(f"Error getting {cls.service_name} statistics: {e}")
            return {}
    
    @classmethod
    def _clear_related_cache(cls, series_id: str = None):
        """清除相关缓存"""
        cache_keys = [
    ...
    def clear_all_cache(cls):
        ...
        try:
            ...
            logger.error(f"Error clearing all {cls.service_name} cache: {e}")
