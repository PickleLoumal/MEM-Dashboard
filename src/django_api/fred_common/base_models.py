...
class BaseFredModel(models.Model):
    ...
    def formatted_value(self):
        ...
        try:
            ...
            if self.unit and "Percent" in self.unit:
                return f"{val:.2f}%"
            if self.unit and "Billion" in self.unit:
                return f"{val:,.1f}B"
            if self.unit and "Million" in self.unit:
                return f"{val:,.1f}M"
            return f"{val:,.2f}"
        except (ValueError, TypeError):
            return str(self.value)

    def calculate_yoy_change(self, previous_value: float | None = None) -> float | None:
        """计算同比变化率"""
    ...
    def get_date_range_data(cls, series_id: str, start_date: str, end_date: str):
        ...
        return cls.objects.filter(
            series_id=series_id, date__gte=start_date, date__lte=end_date
        ).order_by("date")
