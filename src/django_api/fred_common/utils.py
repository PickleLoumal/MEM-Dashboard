...
def format_date(date_input: Union[str, datetime], output_format: str = DISPLAY_DATE_FORMAT) -> str:
    """格式化日期显示"""
    try:
        if isinstance(date_input, str):
            # 尝试解析字符串日期
            date_obj = datetime.strptime(date_input, DEFAULT_DATE_FORMAT)
...
def calculate_yoy_change(current_value: Union[float, Decimal], 
                        previous_value: Union[float, Decimal]) -> Optional[float]:
    """计算同比变化率"""
    try:
...
def calculate_period_change(current_value: Union[float, Decimal], 
                          previous_value: Union[float, Decimal]) -> Optional[float]:
    """计算期间变化率（通用）"""
...
def clean_numeric_value(value: Union[str, float, int, Decimal]) -> Optional[float]:
    """清理并转换数值"""
...
def format_numeric_value(value: Union[float, int, Decimal], 
...
def get_formatted_date_range(days_back: int = 365) -> Tuple[str, str]:
    ...
    return (
        start_date.strftime(DEFAULT_DATE_FORMAT),
        end_date.strftime(DEFAULT_DATE_FORMAT)
    )
...
def parse_frequency(frequency: str) -> str:
    ...
    if freq_lower in ['a', 'annual', 'yearly']:
        return 'Annual'
    elif freq_lower in ['q', 'quarterly']:
        return 'Quarterly'
    elif freq_lower in ['m', 'monthly']:
        return 'Monthly'
    elif freq_lower in ['w', 'weekly']:
        return 'Weekly'
    elif freq_lower in ['d', 'daily']:
        return 'Daily'
    else:
...
def calculate_growth_rate(values: List[Union[float, int]], periods: int = 1) -> List[Optional[float]]:
    """计算增长率序列"""
    if not values or len(values) < periods + 1:
        return []
    
    growth_rates: List[Optional[float]] = [None] * periods  # 前几个值没有足够的历史数据
...
def detect_outliers(values: List[Union[float, int]], threshold: float = 3.0) -> List[bool]:
    """检测异常值（使用标准差方法）"""
...
def retry_on_failure(max_attempts: int = 3, delay: float = 1.0):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if last_exception:
                logger.error(f"Function {func.__name__} failed after {max_attempts} attempts: {last_exception}")
