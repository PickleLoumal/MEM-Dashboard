...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--indicator',
            type=str,
            help='指标名称 (如: cpi, gdp, unemployment)'
        )
        parser.add_argument(
            '--category',
            type=str,
            help='指标类别 (如: inflation, economic_growth)'
        )
        parser.add_argument(
            '--all',
            action='store_true',
    ...
    def _check_api_key(self) -> bool:
        ...
        self.stdout.write(
            self.style.SUCCESS(f'API密钥已配置: {api_key[:8]}...')
        )
        return True
    ...
    def _test_connection(self, fetcher):
        ...
        if result.get('connection_valid'):
            self.stdout.write(
                self.style.SUCCESS('✓ API连接测试成功')
            )
            self.stdout.write(f"配置信息: {result.get('country_config', {}).get('country')}")
        else:
            self.stdout.write(
                self.style.ERROR(f'✗ API连接测试失败: {result.get("error")}')
            )
    ...
    def _get_summary(self, fetcher, options):
        ...
        for indicator, data in summary.get('data', {}).items():
            if 'error' in data:
                self.stdout.write(
                    self.style.WARNING(f'  {indicator}: {data["error"]}')
                )
            else:
                self.stdout.write(
    ...
    def _get_all_indicators(self, fetcher, options):
        ...
        result = fetcher.get_multiple_indicators(
    ...
    def _get_category_data(self, fetcher, options):
        ...
        result = fetcher.get_category_indicators(
            category=category,
            limit=options['limit']
        )
        ...
        if result.get('success'):
            self.stdout.write(
                self.style.SUCCESS(
                    f'✓ 类别数据获取完成 - 成功: {result.get("success_count", 0)}'
                )
            )
        else:
            self.stdout.write(
                self.style.ERROR(f'✗ 类别数据获取失败: {result.get("error")}')
            )
            return
    ...
    def _get_indicator_data(self, fetcher, options):
        ...
        result = fetcher.get_indicator_data(
        ...
        if result.get('success'):
            self.stdout.write(
                self.style.SUCCESS(
                    f'✓ 指标数据获取完成 - {result.get("data_count", 0)}条记录'
                )
            )
            ...
            self.stdout.write(
                self.style.ERROR(f'✗ 指标数据获取失败: {result.get("error")}')
            )
            return
    ...
    def _save_output(self, data, options):
        ...
        if options.get('output_file'):
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✓ 数据已保存到: {options["output_file"]}')
                )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f'✗ 文件保存失败: {e}')
                )
