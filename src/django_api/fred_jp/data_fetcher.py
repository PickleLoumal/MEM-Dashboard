...
class JapanFredDataFetcher(BaseFredDataFetcher):
    ...
    def get_indicator_data(
        ...
        if not indicator_config:
            ...
            return {"success": False, "error": f"无法获取指标配置: {indicator_name}"}
        ...
        if not series_info:
            ...
            return {"success": False, "error": f"无法获取系列信息: {series_id}"}
        ...
        if not observations or len(observations) == 0:
            ...
            return {"success": False, "error": f"数据验证失败或数据不完整: {series_id}"}
        ...
        for obs in observations:
            formatted_obs = {
                ...
                if series_info
                else indicator_config["frequency"],
            }
        ...
        return {
            ...
            "data": formatted_data,
            "fetch_time": datetime.now(tz=UTC).isoformat(),
        }
    ...
    def get_multiple_indicators(
        ...
        for indicator_name in indicator_names:
            try:
                ...
                logger.exception("获取指标数据失败 {indicator_name}")
    ...
    def get_category_indicators(
        ...
        return self.get_multiple_indicators(
            indicator_names=indicators, start_date=start_date, end_date=end_date, limit=limit
        )
    ...
    def get_latest_data_summary(self) -> dict[str, Any]:
        ...
        for indicator_name in all_indicators:
            try:
                # 只获取最新的1条数据
                result = self.get_indicator_data(indicator_name=indicator_name, limit=1)
                ...
                if result.get("success") and result.get("data"):
                    ...
                    summary[indicator_name] = {"error": result.get("error", "数据获取失败")}

            except Exception as e:
                logger.exception("获取{indicator_name}最新数据失败")
                summary[indicator_name] = {"error": str(e)}

        return {
            "country": "Japan",
            "summary_date": datetime.now(tz=UTC).isoformat(),
    ...
    def _calculate_yoy_change_local(self, observations: list[dict]) -> float | None:
        """
        计算年同比变化（本地实现）
