...
class JapanFredConfigManager:
    ...
    @classmethod
    def get_indicator_config(cls, indicator_name: str) -> Optional[Dict]:
        """获取指定指标的配置"""
        return cls.JAPAN_INDICATORS.get(indicator_name.lower())
    
    @classmethod
    def get_series_id(cls, indicator_name: str) -> Optional[str]:
    ...
    def get_indicators_by_category(cls, category: str) -> List[str]:
        """根据类别获取指标列表"""
        return [
            name for name, config in cls.JAPAN_INDICATORS.items()
