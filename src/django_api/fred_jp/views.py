...
from .models import FredJpIndicator
...
class FredJpIndicatorViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_indicator_data(self, series_id: str, limit: int = 100):
        ...
        try:
            # 从日本数据库表获取数据
            observations = FredJpIndicator.objects.filter(series_id=series_id).order_by("-date")[
                :limit
            ]
            ...
            return {"data": data, "observations": observations_data, "meta": meta}

        except Exception:
            logger.exception("Error fetching Japan indicator data for %s", series_id)
            return None
    ...
    def get_indicator(self, request, indicator_name=None):
        ...
        try:
            ...
            logger.exception("Error in Japan get_indicator for %s", indicator_name)
            return Response(
                {
                    "success": False,
