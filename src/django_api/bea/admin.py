...
class BeaSeriesInfoAdmin(admin.ModelAdmin):
    """BEA 系列信息管理"""
    list_display = ('series_id', 'title', 'category', 'frequency', 'units', 'table_name', 'last_updated')
    ...
    fieldsets = (
        ...
        ('技术信息', {
            'fields': ('frequency', 'table_name', 'line_number', 'line_description', 'dataset_name')
        }),
        ('其他', {
...
class BeaIndicatorAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('数据信息', {
            'fields': ('time_period', 'value', 'date', 'unit', 'frequency')
        }),
        ('技术信息', {
...
class BeaIndicatorConfigAdmin(admin.ModelAdmin):
    """BEA 指标配置管理"""
    list_display = ('series_id', 'name', 'category', 'priority', 'is_active', 'auto_fetch', 'updated_at')
    ...
    fieldsets = (
        ('基本配置', {
            'fields': ('series_id', 'name', 'description', 'category', 'api_endpoint')
        }),
        ('BEA API 配置', {
            'fields': ('table_name', 'line_description', 'line_number', 'units', 'frequency', 'years', 'dataset_name')
        }),
        ('系统配置', {
            'fields': ('priority', 'is_active', 'auto_fetch', 'fallback_value')
        }),
        ('元数据', {
            'fields': ('created_by', 'updated_by', 'additional_config'),
            'classes': ('collapse',)
        }),
        ('时间戳', {
            'fields': ('created_at', 'updated_at'),
