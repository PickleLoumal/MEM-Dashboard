...
class DynamicBeaConfigManager:
    ...
    def get_all_indicators(cls):
        ...
        try:
            ...
            logger.error(f"Error loading BEA configs from database: {e}")
    ...
    def get_indicator_config(cls, series_id):
        ...
        try:
            config = BeaIndicatorConfig.objects.filter(
                series_id=series_id, 
                is_active=True
            ).first()
    ...
    def update_indicator(cls, series_id, config_data):
        ...
        try:
            ...
            logger.error(f"Indicator config not found: {series_id}")
            return None
        except Exception as e:
            logger.error(f"Error updating indicator config {series_id}: {e}")
            raise
    ...
    def get_statistics(cls):
        ...
        try:
            ...
            auto_fetch = BeaIndicatorConfig.objects.filter(
                is_active=True, 
                auto_fetch=True
            ).count()
            
            categories = BeaIndicatorConfig.objects.filter(
                is_active=True
    ...
    def clear_all_cache(cls):
        ...
        try:
            ...
            all_categories = BeaIndicatorConfig.objects.values_list('category', flat=True).distinct()
