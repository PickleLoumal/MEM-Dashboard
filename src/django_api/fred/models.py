"""
Django Models for MEM Dashboard FRED API
映射现有PostgreSQL数据库表，不进行迁移以避免冲突
"""
...
class FredSeriesInfo(models.Model):
    """FRED系列信息表 - 映射fred_series_info表"""
    ...
    class Meta:
        db_table = 'fred_series_info'  # 使用现有表
...
class FredIndicator(models.Model):
    """FRED指标数据表 - 映射fred_indicators表"""
    ...
    class Meta:
        db_table = 'fred_indicators'  # 使用现有表
        managed = False  # Django不管理这个表，避免migration冲突
