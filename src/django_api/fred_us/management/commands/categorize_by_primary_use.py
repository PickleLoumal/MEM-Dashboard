...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--mark-unused", action="store_true", help="将前端未使用的指标标记为Unused"
        )

    def handle(self, *args, **options):
        """处理命令执行"""
        try:
            self.stdout.write(self.style.HTTP_INFO("基于主要用途的前端指标分类工具"))
    ...
    def _show_results(self, results):
        """显示更新结果"""
        self.stdout.write("\n" + "=" * 50)
        self.stdout.write(self.style.HTTP_INFO("主要用途分类更新完成!"))
    ...
    def _verify_final_categorization(self):
        ...
        if categorized_count == 49:  # 前端使用的指标数量
            self.stdout.write(self.style.SUCCESS("✓ 所有前端指标都已正确分类!"))
        else:
            self.stdout.write(
