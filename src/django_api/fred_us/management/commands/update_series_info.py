...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--dry-run", action="store_true", help="只显示缺失的指标，不实际更新")
        parser.add_argument("--limit", type=int, default=None, help="限制处理的指标数量")
        parser.add_argument("--delay", type=float, default=1.0, help="API调用间隔延迟（秒）")

    def handle(self, *args, **options):
        """处理命令执行"""
        try:
            self.stdout.write(self.style.HTTP_INFO("FRED US Series Info 数据库整理工具"))
            ...
            if not missing_series:
                self.stdout.write(self.style.SUCCESS("✓ 所有指标的Series Info已完整，无需更新!"))
                return
            ...
            # 执行批量更新
            _results = self._fetch_and_save_series_info(missing_series, delay=options["delay"])
    ...
    def _get_missing_series_ids(self) -> list[str]:
        ...
        series_info_series = set(FredUsSeriesInfo.objects.values_list("series_id", flat=True))

        missing_series = indicators_series - series_info_series
        return sorted(missing_series)

    def _fetch_and_save_series_info(
        self, series_ids: list[str], delay: float = 1.0
    ) -> dict[str, Any]:
        """批量获取并保存系列信息"""
        results = {"total": len(series_ids), "successful": 0, "failed": 0, "errors": []}
        ...
        try:
            ...
            for i, series_id in enumerate(series_ids, 1):
                try:
                    self.stdout.write(
                        f"[{i:2d}/{len(series_ids)}] 获取 {series_id} 的系列信息...", ending=""
                    )
                    ...
                    if series_info:
                        ...
                        if success:
                            ...
                            self.stdout.write(self.style.ERROR(" ✗ 保存失败"))
                    else:
                        results["failed"] += 1
                        error_msg = f"API未返回数据: {series_id}"
                        results["errors"].append(error_msg)
                        self.stdout.write(self.style.ERROR(" ✗ API无数据"))
                    ...
                    results["failed"] += 1
                    error_msg = f"处理 {series_id} 时出错: {e!s}"
                    results["errors"].append(error_msg)
                    self.stdout.write(self.style.ERROR(f" ✗ 错误: {e!s}"))
                    continue

            self.stdout.write("=" * 60)
            self.stdout.write(self.style.HTTP_INFO("批量更新完成!"))
            self.stdout.write(f"总计: {results['total']} 个指标")
            self.stdout.write(self.style.SUCCESS(f"成功: {results['successful']} 个"))
            if results["failed"] > 0:
                self.stdout.write(self.style.ERROR(f"失败: {results['failed']} 个"))
            ...
            self.stdout.write(self.style.ERROR(f"批量更新过程中发生错误: {e}"))
            return results

    def _verify_update(self) -> None:
        """验证更新结果"""
        self.stdout.write("\n" + "=" * 60)
        self.stdout.write(self.style.HTTP_INFO("验证更新结果:"))
        ...
        if indicators_count == series_info_count:
            self.stdout.write(self.style.SUCCESS("✓ 所有指标的Series Info已完整!"))
        else:
            missing_series = self._get_missing_series_ids()
            self.stdout.write(
