...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--verbose", action="store_true", help="显示详细的分类过程")

    def handle(self, *args, **options):
        """处理命令执行"""
        try:
            self.stdout.write(self.style.HTTP_INFO("FRED US Series Info Dashboard组件分类工具"))
    ...
    def _show_categorization_plan(self, categorization_map, verbose=False):
        ...
        total_indicators = 0
        for _category_key, category_info in categorization_map.items():
        ...
        if uncategorized:
            self.stdout.write(self.style.WARNING(f"  未分类指标: {len(uncategorized)}个"))
            if verbose:
            ...
            self.stdout.write(self.style.SUCCESS("  ✓ 所有指标都已分类"))

    def _update_categories(self, categorization_map):
        """执行分类更新"""
        results = {"updated": 0, "errors": 0, "categories": {}, "error_details": []}
    ...
    def _verify_final_categorization(self):
        ...
        # 统计各分类的数量
        categories = (
            FredUsSeriesInfo.objects.values("category")
        ...
        if uncategorized_count == 0:
            self.stdout.write(self.style.SUCCESS("✓ 所有指标都已正确分类!"))
        else:
            self.stdout.write(self.style.WARNING(f"✗ 仍有 {uncategorized_count} 个指标未分类"))
