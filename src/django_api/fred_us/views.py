...
class FredUsIndicatorViewSet(
    ...
    data_fetcher: UsFredDataFetcher | None
    ...
    def list(self, request: Request) -> Response:
        ...
        try:
            ...
            last_updated: datetime | None = FredUsIndicator.objects.aggregate(Max("updated_at"))[
                "updated_at__max"
            ]
            ...
            logger.exception("获取美国FRED API概览失败")
            return self._error_response(f"API概览获取失败: {e!s}")

    def get_data_fetcher(self) -> UsFredDataFetcher | None:
        """获取数据获取器实例 (延迟初始化)"""
        if not self.data_fetcher:
            try:
                self.data_fetcher = UsFredDataFetcher()
            except Exception:
                logger.exception("美国FRED数据获取器初始化失败")
                return None
    ...
    def indicator(self, request: Request) -> Response:
        ...
        try:
            ...
            latest_record = (
                FredUsIndicator.objects.filter(series_id=series_id).order_by("-date").first()
            )
            ...
            if not latest_record:
                ...
                if api_result.get("success"):
                    latest_record = (
                        FredUsIndicator.objects.filter(series_id=series_id)
                        .order_by("-date")
                        .first()
                    )
            ...
            observations = FredUsIndicator.objects.filter(series_id=series_id).order_by("-date")[
                :limit
            ]
            ...
            logger.exception("获取美国FRED指标数据失败")
            return self._error_response(f"服务器内部错误: {e!s}")
    ...
    def status(self, request: Request) -> Response:
        ...
        try:
            ...
            logger.exception("获取美国FRED状态失败")
            return self._error_response(f"状态检查失败: {e!s}")
    ...
    def health(self, request: Request) -> Response:
        ...
        try:
            ...
            logger.exception("美国FRED健康检查失败")
            return self._error_response(f"健康检查失败: {e!s}")
    ...
    def fetch_data(self, request: Request) -> Response:
        ...
        try:
            ...
            return self._error_response(f"获取美国指标数据失败: {indicator_name}", details=result)

        except Exception as e:
            logger.exception("手动获取美国FRED数据失败")
            return self._error_response(f"数据获取失败: {e!s}")
