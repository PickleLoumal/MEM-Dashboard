...
# 添加Django项目路径 - 使用动态路径
# 当前文件: fred_us/migrate_to_auto_config.py -> django_api/
DJANGO_API_DIR = Path(__file__).resolve().parent.parent
sys.path.append(str(DJANGO_API_DIR))
...
from fred_us.models import FredUsIndicatorConfig, FredUsSeriesInfo
...
def create_fred_us_configs():
    ...
    for info in series_infos:
        ...
        _, created = FredUsIndicatorConfig.objects.update_or_create(
            series_id=info.series_id, defaults=config_data
        )
