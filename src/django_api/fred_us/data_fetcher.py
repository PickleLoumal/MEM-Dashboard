...
class UsFredDataFetcher(BaseFredDataFetcher):
    ...
    def save_series_info(self, series_id: str, series_info: dict) -> bool:
        """保存系列信息到美国FRED数据库"""
        try:
            _series_obj, created = FredUsSeriesInfo.objects.update_or_create(
                series_id=series_id,
                defaults={
            ...
            logger.exception("保存美国系列信息失败 {series_id}")
            return False
        ...
        try:
            for observation in observations:
                ...
                if date and value and value != ".":
                    try:
                        _indicator_obj, created = FredUsIndicator.objects.update_or_create(
                            series_id=series_id,
                            date=date,
                            defaults={
            ...
            logger.exception("保存美国观测数据失败 {series_id}")
            return 0
    ...
    def get_latest_data_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                "last_updated": FredUsIndicator.objects.aggregate(Max("updated_at")).get(
                    "updated_at__max"
                ),
