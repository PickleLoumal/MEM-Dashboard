...
class FredUsAutoFetcher:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.api_key = api_key or getattr(settings, 'FRED_API_KEY', None) or os.environ.get('FRED_API_KEY')
    ...
    def fetch_single_indicator(self, config: FredUsIndicatorConfig) -> bool:
        ...
        try:
            ...
            logger.error(f"获取指标数据异常: {config.series_id} - {e}")
    ...
    def fetch_by_frequency(self, frequency: str = 'daily') -> Dict[str, Any]:
        ...
        for series_id, config_dict in configs.items():
            # 获取数据库中的配置对象
            try:
    ...
    def fetch_all_auto_indicators(self) -> Dict[str, Any]:
        ...
        if not configs:
            ...
            return {
                'status': 'no_configs',
                'message': 'No auto-fetch configs found'
            }
    ...
    def fetch_specific_indicators(self, series_ids: List[str]) -> Dict[str, Any]:
        ...
        for series_id in series_ids:
            try:
                ...
                logger.error(f"获取指标失败 {series_id}: {e}")
                results[series_id] = {
                    'success': False,
                    'error': str(e)
                }
        ...
        summary = {
            ...
            'total_processed': len([r for r in results.values() if 'error' not in r or r['error'] != 'Configuration not found or inactive']),
        ...
        logger.info(f"指定指标数据获取完成: {summary['successful']}/{summary['total_requested']} 成功")
        return summary
        ...
        try:
            ...
            for config in recent_configs:
                recent_fetches.append({
                    ...
                    'last_fetch_time': config.last_fetch_time.isoformat() if config.last_fetch_time else None,
...
# 工厂函数
def get_fred_us_auto_fetcher(api_key: Optional[str] = None):
    """获取美国FRED自动获取器实例"""
