...
from .models import FredUsIndicator, FredUsSeriesInfo, FredUsIndicatorConfig
...
class FredUsSeriesInfoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('series_id', 'title', 'category')
        }),
        ('技术信息', {
            'fields': ('units', 'frequency', 'seasonal_adjustment'),
            'classes': ('collapse',)
        }),
        ('说明', {
...
class FredUsIndicatorAdmin(admin.ModelAdmin):
    """美国FRED指标数据管理界面"""
    list_display = ('series_id', 'indicator_name', 'indicator_type', 'date', 'value', 'source', 'updated_at')
...
class FredUsIndicatorConfigAdmin(admin.ModelAdmin):
    """美国FRED指标配置管理界面"""
    list_display = ('series_id', 'name', 'category', 'auto_fetch', 'fetch_frequency', 'priority', 'is_active', 'fetch_status', 'last_fetch_time')
    list_filter = ('auto_fetch', 'fetch_frequency', 'priority', 'is_active', 'fetch_status', 'category')
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('series_id', 'name', 'category', 'description')
        }),
        ('自动抓取配置', {
            'fields': ('auto_fetch', 'fetch_frequency', 'priority', 'is_active')
        }),
        ('抓取状态', {
            'fields': ('fetch_status', 'last_fetch_time'),
            'classes': ('collapse',)
        }),
        ('高级配置', {
