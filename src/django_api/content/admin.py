...
class ContentCategoryAdmin(admin.ModelAdmin):
    ...
    def content_count(self, obj):
        ...
        if count > 0:
            ...
            return format_html(
                '<a href="{}?category={}" style="color: #0066cc;">{} items</a>', url, obj.id, count
            )
        return "0 items"
...
class ModalContentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Content Details",
            {"fields": ("description", "importance", "source"), "classes": ("wide",)},
        ),
        (
            "Structured Data",
            {
    ...
    def breakdown_preview(self, obj):
        if obj.breakdown_data:
            try:
                ...
                return mark_safe(f'<span style="color: #d32f2f;">Data parsing error: {e!s}</span>')
    ...
    def make_active(self, request, queryset):
        ...
        self.message_user(
            request, f"Successfully activated {count} content items", level=messages.SUCCESS
        )
    ...
    def make_inactive(self, request, queryset):
        ...
        self.message_user(
            request, f"Successfully deactivated {count} content items", level=messages.SUCCESS
        )
