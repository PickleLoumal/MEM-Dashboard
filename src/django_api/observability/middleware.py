...
frontend_trace_id_var: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "frontend_trace_id", default=None
)
frontend_span_id_var: contextvars.ContextVar[str | None] = contextvars.ContextVar(
...
def get_frontend_trace_id() -> str | None:
    """获取当前请求的前端 Trace ID"""
    return frontend_trace_id_var.get()


def get_frontend_span_id() -> str | None:
    """获取当前请求的前端 Span ID"""
...
class OpenTelemetryMiddleware:
    ...
    def _process_request(
        self, request: HttpRequest, frontend_trace_id: str, frontend_span_id: str, request_id: str
    ) -> HttpResponse:
        # Start timing
