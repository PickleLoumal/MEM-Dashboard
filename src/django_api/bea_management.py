...
def create_sample_configs():
    ...
    sample_configs = [
        {
            ...
            "dataset_name": "NIPA",
            "additional_config": {"seasonally_adjusted": True, "data_source": "BEA NIPA Tables"},
        },
        {
    ...
    for config_data in sample_configs:
        try:
            ...
            logger.exception("创建配置失败 {config_data['series_id']}")
...
def list_all_configs():
    ...
    for config in configs:
        ...
        logger.info(
            f"{config.series_id:<20} {config.name[:28]:<30} {config.category:<20} {status_str:<8} {config.priority or 'N/A':<8}"
        )
...
def show_api_endpoints():
    ...
    try:
        ...
        logger.info(
            f"总计: {len(management_endpoints) + len(all_configs) + len(legacy_endpoints)} 个端点"
        )

    except Exception:
        logger.exception("获取API端点失败")
