...
BO_PATTERNS = {
    'fiscal_year': re.compile(
        r'(?:FY\s?(\d{4})|fiscal year[- ]?end[:\s]*([A-Za-z]+\s+\d{1,2}))',
        re.IGNORECASE
    ),
    'revenue': re.compile(
    ...
    'sales_standalone': re.compile(
        r'(?:^|[;:,]\s*)sales\s+([\d.]+\s*[BMT]?\s*(?:CNY|USD|RMB|billion|million)?)',
        re.IGNORECASE
    ),
    ...
    'nim': re.compile(
        r'(?:net\s+interest\s+margin|NIM)\s*(?:\(NIM\))?\s*(?:of\s+)?([\d.]+%)',
        re.IGNORECASE
    ),
