...
from .parser import extract_ai_content_sections, extract_sources_from_key_takeaways, parse_business_overview_to_json
from .utils import XAI_API_KEY, safe_decimal, format_market_cap, get_stock_data_sync
...
@sync_to_async
def save_summary_to_db_async(company_obj, summary_data: Dict) -> Optional[bool]:
    """异步保存摘要到数据库"""
    try:
        ...
        logger.error(f"DB Error for {company_obj.name}: {e}")
        return None
    ...
    for company, task in tasks:
        if task is None:
            stock_data_map[company.id] = {
                'last_price': None, 'market_cap': None, 'currency': '', 'success': False
            }
        else:
            try:
                result = await task
...
async def process_company_ai(
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.error(f"AI Error for {company_name} (Attempt {attempt+1}): {e}")
