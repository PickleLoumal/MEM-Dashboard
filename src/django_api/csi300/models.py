...
class CSI300HSharesCompany(models.Model):
    ...
    ticker = models.CharField(max_length=20, blank=True, null=True, help_text="Stock ticker")
    region = models.CharField(
        max_length=50, blank=True, null=True, help_text="Region (e.g., Hong Kong)"
    )
    im_sector = models.CharField(
        max_length=150,
        blank=True,
        null=True,
        help_text="IM Sector (combined from IM Code and Industry)",
    )
    industry = models.CharField(
        max_length=500, blank=True, null=True, help_text="Industry (Sub-industry from CSV)"
    )
    gics_industry = models.CharField(
        max_length=100, blank=True, null=True, help_text="GICS Industry"
    )
    ...
    # Price Information
    price_local_currency = models.DecimalField(
        max_digits=20, decimal_places=6, blank=True, null=True, help_text="Price in local currency"
    )
    previous_close = models.DecimalField(
        max_digits=20, decimal_places=6, blank=True, null=True, help_text="Previous close price"
    )
...
class CSI300InvestmentSummary(models.Model):
    ...
    company = models.OneToOneField(
        CSI300Company, on_delete=models.CASCADE, related_name="investment_summary"
    )
