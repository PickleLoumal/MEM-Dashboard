...
class NavigationController {
    ...
    async loadModalDebtChart() {
        ...
        if (!chartCanvas) {
            console.log('Modal chart canvas not found');
            return;
        }
        
        try {
            console.log('Loading modal debt chart...');
            ...
            if (valueIndicator) {
                valueIndicator.textContent = `${currentValue.toFixed(2)}%`;
            }
    ...
    async loadModalMotorVehiclesChart() {
        ...
        if (!chartCanvas) {
            console.log('Modal motor vehicles chart canvas not found');
            return;
        }
        
        try {
            console.log('Loading motor vehicles chart from BEA API...');
            ...
            if (!response || !response.success || !response.data) {
                throw new Error('Failed to fetch motor vehicles data from BEA API');
            }
            ...
            console.error('Error loading motor vehicles chart from BEA API:', error);
...
// Store navigation controller globally
window.navigationController = navigationController;
window.investmentCardController = investmentCardController;
